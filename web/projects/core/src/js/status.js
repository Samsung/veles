  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      workflows.forEach(function(pair) {
        ...
        let slaves = Object.keys(workflow.slaves).map(function(key) {
        ...
        var jobs = 0;
        for (let slave of workflow.slaves) {
          jobs += slave.value.jobs;
        }
      ...
      let free_svgs = [];
      for (let key in svg_cache) {
        if (typeof listed_workflows[key] === "undefined") {
          free_svgs.push(key);
        }
      }
      for (let key in free_svgs) {
        delete svg_cache[key];
      }
      let objs = $.parseHTML(items);
...
global.activateListItem = function(item_id) {
  ...
  $("#workflow-log-id").text(workflow.log_id).attr("href", "logs.html?session=" + workflow.log_id);
