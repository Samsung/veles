...
function setupUI() {
  ...
  let sss = $("select.slave-setter");
  if (nodes.length > 1) {
    sss.children().first().attr("selected", "selected");
    sss.selectmenu("refresh").selectmenu("widget").removeAttr("style");
  }
...
function setupLogTables() {
  ...
  widgetOptions: {
      ...
      stickyHeaders_attachTo: logs_master
  },
  sortList: [[0, 0]],
  textExtraction: {
        0: function (node) {
  ...
  logs_master.scroll(function(event) {
    ...
    let offset = $("#logs-master").scrollTop();
    let rows = table_rows.master.pos;
    let left = 0;
    ...
    let slave_row = $("#logs-contents-slave > tbody > tr");
    if (left > 0) {
      if (slave_list[0][1] > 0) {
        left = rows.length - left;
      }
      offset = $(slave_row[left - 1]).position().top - $("thead").height();
      if (offset < 0) {
        offset = 0;
      }
    } else {
      if (slave_list[0][1] > 0) {
        offset = $(slave_row[rows.length - 1]).position().top - $("thead").height();
      } else {
        offset = 0;
      }
    }
  ...
  $("#sync-logs").data()['ui-checkbox'].button.on("click", function(event) {
    $("#logs-master").trigger("scroll");
  });
}
...
function scrollSlaveLogsDiv(offset) {
  let logs_slave = $("#logs-slave");
  let pos = logs_slave.scrollTop();
  if (pos != offset) {
    if (!animating_scroll.slave) {
      animating_scroll.slave = true;
      logs_slave.animate({scrollTop: offset}, 500, function() {
...
function scrollLogs(node) {
  ...
  let log_row = $("#logs-contents-" + node + " > tbody > tr");
  if (log_row.length == 0) {
    return;
  }
  let offset = $(log_row[left - 1]).position().top - $("thead").height();
  let logs_node = $("#logs-" + node);
  if (logs_node.scrollTop() != offset) {
    animating_scroll[node] = true;
    sync_scroll_block[node] = true;
    logs_node.animate({scrollTop: offset}, 500, function() {
...
global.renderLogs = function(logs, instance) {
