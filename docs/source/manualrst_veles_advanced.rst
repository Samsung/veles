...
Here is the description of what happens after you execute velescli.py:
    ...
    #. A new instance of :class:`velescli.Main <scripts.velescli.Main>` class is created and
       it's :func:`run() <scripts.velescli.Main.run>` method is called.
    ...
    #. "Special" command line arguments are checked for presence. They are listed in
       :const:`Main.SPECIAL_OPTS <scripts.velescli.Main.SPECIAL_OPTS>` and include
    ...
    #. :meth:`_main` returns at once if ``--dry-run`` was set to "init", otherwise
       it creates an OpenCL device handle, unless ``--disable-opencl`` was passed.
...
.. include:: manualrst_veles_config.rst