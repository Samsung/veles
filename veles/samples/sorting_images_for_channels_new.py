    ...
    def __init__(self, parent=None):
        ...
        for root, dirs, files in os.walk(thumbs_path):
            for i, nme in enumerate(dirs):
                if cnndir.search(nme) is not None or abcdir.search(nme) is\
                                        not None\
                                        ...
                                        or no_channeldir.search(nme) is\
                                        not None\
        ...
        if len(self.thumbs) == 2:
            ...
            self.button7.clicked.connect(lambda:\
                                        self.cbsbr(self.thumbs))
            self.button35.clicked.connect(lambda:\
                                        self.cbsbl(self.thumbs))
