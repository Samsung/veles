t0 = {"S0": 0,
    ...
    'data_set': {
        ...
        'type1': 1,
        'var1': {
            'use_rand_for_train_data': 1,
            'use_rand_for_valid_data': 1,
            'use_rand_for_test_data': 0,
            'divideblock': 1,
            'size_fix_div': {'sizeTR ': range(1, 5), 'sizeV': range(5, 9), \
                             'sizeTE': range(9, 13)},
            'size_proc_div': {'sizeTR%': 0.7, 'sizeV%': 0.3}  # 'sizeTE%':0.15}
             },
        'not_merge_type': 0,
        'index_file': {
                  'ind_fileTR': [1],
    ...
    'use_norm': 5,
    'norm': {'d_1': 1, 'min': (-1), 'max': 1, 'minmax': 1, \
             'meanstd': 1, 'removeconstantrows': 0},
    },
    'train_param': {
                   'type': 'BP1',
                   'global_alpha': 0.9,
